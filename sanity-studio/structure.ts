...
const postList = (S: Parameters<StructureResolver>[0], title: string, filter: string) => S.listItem()
  .title(title)
  .child(S.documentList().title(title).schemaType('post').filter(filter))

export const structure: StructureResolver = (S) => S.list()
  .title('内容管理')
  .items([
    postList(S, '全部文章', '_type == "post"'),
