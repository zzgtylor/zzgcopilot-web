...
export function CheckoutButton({ slug }: { slug: string }) {
  ...
  return <div className="mt-5"><button onClick={checkout} data-analytics-event="cta" data-analytics-label="订阅并阅读" className="rounded bg-[var(--site-primary)] px-5 py-2.5 text-white">订阅并阅读</button>{message && <p className="mt-2 text-sm text-red-700">{message}</p>}</div>
}
