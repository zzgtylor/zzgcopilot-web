...
const components: PortableTextComponents = {
  types: {
    ...
    callout: ({ value }) => {
      const toneClass = value.tone === 'warning'
        ...
        : value.tone === 'important'
          ...
          : 'border-[var(--site-primary)] bg-[#eef6fb] text-[#17394e]'
    ...
    tutorialStep: ({ value }) => <section className="my-7 rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><strong className="text-[var(--site-primary)]">第 {value.stepNumber} 步 · {value.title}</strong><p className="mb-0 mt-2 whitespace-pre-wrap text-gray-700">{value.text}</p></section>,
    ...
    download: ({ value }) => {
      ...
      return href ? <p className="my-7"><a href={href} className="inline-flex rounded bg-[var(--site-primary)] px-5 py-3 font-medium text-white no-underline hover:bg-[var(--site-secondary)]" download={Boolean(value.fileUrl)}>{value.label || '下载文件'}</a></p> : null
    },
    actionButton: ({ value }) => value.href ? <p className="my-7"><a href={value.href} className={value.style === 'secondary' ? 'inline-flex rounded border border-[var(--site-primary)] px-5 py-3 font-medium text-[var(--site-primary)] no-underline hover:bg-[#eef6fb]' : 'inline-flex rounded bg-[var(--site-primary)] px-5 py-3 font-medium text-white no-underline hover:bg-[var(--site-secondary)]'} target={String(value.href).startsWith('/') ? undefined : '_blank'} rel={String(value.href).startsWith('/') ? undefined : 'noreferrer'}>{value.label || '了解更多'}</a></p> : null,
    videoEmbed: ({ value }) => {
