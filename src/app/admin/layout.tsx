        ...
        <div className="min-h-screen bg-gray-50">
              <aside className="fixed top-0 left-0 h-full w-56 bg-gray-900 text-white flex flex-col z-20">
                      <div className="px-6 py-5 border-b border-gray-700">
                                <Link href="/" className="text-lg font-bold text-white">ZZGCopilot</Link>Link>
                                <p className="text-xs text-gray-400 mt-0.5">管理后台</p>p>
                      </div>div>
                      ...
                      <div className="px-4 py-4 border-t border-gray-700">
                                <p className="text-xs text-gray-400 truncate">{session.user.email}</p>p>
                                <form action="/api/auth/signout" method="POST">
                                            <button type="submit" className="mt-2 text-xs text-gray-400 hover:text-white transition">退出登录</button>button>
                                </form>form>
                      </div>div>
              </aside>aside>
