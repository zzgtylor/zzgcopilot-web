...
export default async function AnalyticsAdmin({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  const email = await adminEmail(requestHeaders)
  ...
  return <main className="min-h-screen bg-gray-50 px-5 py-8 text-gray-900"><div className="mx-auto max-w-7xl"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-gray-500">管理员：{email}</p><h1 className="mt-1 text-3xl font-bold">网站数据报表</h1></div><div className="flex flex-wrap gap-2">{[1, 7, 30, 90, 365].map(value => <Link key={value} href={`/admin/analytics?range=${value}`} className={value === range ? 'rounded bg-[var(--site-primary)] px-3 py-2 text-sm text-white' : 'rounded border bg-white px-3 py-2 text-sm'}>{value === 1 ? '今天' : `${value} 天`}</Link>)}<a href={`/api/reports/analytics.csv?range=${range}`} className="rounded border bg-white px-3 py-2 text-sm">导出 CSV</a><form action="/api/admin/logout" method="post"><button className="rounded border bg-white px-3 py-2 text-sm">退出管理员后台</button></form></div></header>
