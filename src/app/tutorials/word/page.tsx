...
export const metadata: Metadata = {
  title: 'Word 从入门到精通（完整版）- ZZGCopilot',
  description: '出书级 Microsoft Word 完整教程：5 篇 20 章，从界面认识、字符段落排版、样式主题、长文档工程，到表格图形、邮件合并、修订协作、域与 VBA 自动化，每节含概念原理、分步操作、实战案例、避坑与进阶技巧。',
}
...
const s = {
  ...
  backBtn: { color: '#fff', textDecoration: 'none', background: 'rgba(255,255,255,0.15)', padding: '6px 16px', borderRadius: 20, fontSize: 13 },
  hero: { background: 'linear-gradient(135deg,#1a56db 0%,#6366f1 100%)', color: '#fff', padding: '56px 24px 64px' },
  heroInner: { maxWidth: 1100, margin: '0 auto' },
  heroTags: { display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 24 },
  tag: { background: 'rgba(255,255,255,0.2)', color: '#fff', padding: '4px 14px', borderRadius: 20, fontSize: 13, fontWeight: 500 },
  heroTitle: { fontSize: 46, fontWeight: 800, margin: '8px 0 14px', letterSpacing: -1, lineHeight: 1.15 },
  heroDesc: { fontSize: 18, opacity: 0.92, maxWidth: 760, lineHeight: 1.8, margin: '0 0 32px' },
  statsRow: { display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, maxWidth: 680 },
  ...
  statLabel: { fontSize: 12, opacity: 0.85, marginTop: 6 },
  content: { maxWidth: 1100, margin: '0 auto', padding: '40px 24px 60px' },
  intro: { background: '#fff', borderRadius: 20, padding: '28px 32px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 32, fontSize: 15, lineHeight: 1.9, color: '#374151' },
  introTitle: { fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 12 },
  partCard: { background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 28 },
  partHeader: { padding: '20px 32px', background: 'linear-gradient(135deg,#eff6ff,#e0e7ff)', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 14 },
  partBadge: { width: 44, height: 44, borderRadius: 12, background: '#1a56db', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, flexShrink: 0 },
  partTitle: { fontSize: 18, fontWeight: 700, color: '#1e3a8a' },
  partDesc: { fontSize: 13, color: '#6b7280', marginTop: 2 },
  chapterList: { padding: '8px 0' },
  chapterRow: { display: 'flex', alignItems: 'flex-start', gap: 16, padding: '16px 32px', textDecoration: 'none', color: 'inherit', borderBottom: '1px solid #f3f4f6', transition: 'background 0.15s' },
  chNum: { width: 40, height: 40, borderRadius: 10, background: '#dbeafe', color: '#1e40af', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, flexShrink: 0 },
  chBody: { flex: 1, minWidth: 0 },
  chTitle: { fontSize: 16, fontWeight: 700, color: '#111827', marginBottom: 4 },
  chTopics: { fontSize: 13, color: '#6b7280', lineHeight: 1.7 },
  chArrow: { color: '#9ca3af', fontSize: 20, alignSelf: 'center', flexShrink: 0 },
  footer: { borderTop: '1px solid #e5e7eb', background: '#fff', padding: '24px', textAlign: 'center', color: '#9ca3af', fontSize: 13 },
}

const parts = [
  {
    no: '一', title: '基础入门篇', desc: '认识 Word、管理文档、掌握文本编辑的核心操作',
    chapters: [
      { n: 1, slug: 'ch1', title: '初识 Word 与工作环境', topics: 'Word 的定位与版本演进 · 界面全解剖 · 功能区与快速访问工具栏 · 视图模式 · 选项设置与个性化' },
      { n: 2, slug: 'ch2', title: '文档的创建、保存与管理', topics: '新建与模板 · 保存格式详解（docx/doc/pdf/odt）· 自动保存与恢复 · 版本历史 · 文件属性与元数据' },
      { n: 3, slug: 'ch3', title: '文本录入与基础编辑', topics: '输入与光标操作 · 插入与改写 · 撤销重做 · 文本选择全套方法 · 剪贴板与粘贴选项 · 查找与替换/通配符' },
    ],
  },
  {
    no: '二', title: '排版核心篇', desc: '从字符到段落、从样式到主题，构建专业排版能力',
    chapters: [
      { n: 4, slug: 'ch4', title: '字符格式精讲', topics: '字体与中西文搭配 · 字号字形 · 字符间距与缩放 · 文字效果 · 突出显示与字符边框底纹' },
      { n: 5, slug: 'ch5', title: '段落格式精讲', topics: '对齐方式 · 缩进 · 行距与段距 · 项目符号与编号 · 多级列表 · 制表位 · 段落边框与底纹' },
      { n: 6, slug: 'ch6', title: '样式系统', topics: '样式的价值 · 内置样式 · 修改与新建样式 · 样式集与管理 · 样式检查器与格式刷' },
      { n: 7, slug: 'ch7', title: '主题、模板与文档外观', topics: '主题/配色/字体集 · 页面颜色与水印 · 创建与复用模板（dotx）' },
    ],
  },
  {
    no: '三', title: '页面与长文档篇', desc: '驾驭页面布局、分节、页眉页脚与引用体系，胜任书籍论文',
    chapters: [
      { n: 8, slug: 'ch8', title: '页面布局与设置', topics: '纸张/页边距/方向 · 分栏 · 稿纸与网格 · 中文公文版式规范' },
      { n: 9, slug: 'ch9', title: '分页符、分节符与多节文档', topics: '各类分隔符 · 节的概念 · 同一文档不同节使用不同版式' },
      { n: 10, slug: 'ch10', title: '页眉页脚与页码', topics: '首页/奇偶页不同 · 多节独立页眉 · 域与 StyleRef · 罗马数字+阿拉伯数字复杂页码方案' },
      { n: 11, slug: 'ch11', title: '自动目录与引用体系', topics: '标题样式与目录 · 图表目录 · 脚注尾注 · 题注与交叉引用 · 书签 · 索引' },
      { n: 12, slug: 'ch12', title: '长文档写作工程', topics: '大纲视图 · 导航窗格 · 主控文档与子文档 · 章节编号体系' },
    ],
  },
  {
    no: '四', title: '对象与数据篇', desc: '表格、图片图形、邮件合并，让文档承载丰富信息',
    chapters: [
      { n: 13, slug: 'ch13', title: '表格制作与数据呈现', topics: '创建与编辑 · 合并拆分 · 尺寸与对齐 · 表格样式 · 公式与排序 · 文本与表格互转' },
      { n: 14, slug: 'ch14', title: '图片与图形', topics: '插入与裁剪 · 环绕与定位 · 颜色与艺术效果 · 压缩 · 形状与文本框 · SmartArt · 图表' },
      { n: 15, slug: 'ch15', title: '邮件合并', topics: '数据源准备 · 主文档 · 合并域与规则 · 信函/标签/信封 · 批量生成与排错' },
    ],
  },
  {
    no: '五', title: '协作、自动化与精通篇', desc: '审阅协作、文档安全、域与 VBA，迈向 Word 高手',
    chapters: [
      { n: 16, slug: 'ch16', title: '修订、批注与协作审阅', topics: '修订模式 · 批注与回复 · 接受拒绝 · 文档比较与合并 · 多人共同创作' },
      { n: 17, slug: 'ch17', title: '文档保护与安全', topics: '限制编辑 · 密码加密 · 数字签名 · 检查文档/兼容性/辅助功能' },
      { n: 18, slug: 'ch18', title: '域代码与自动化字段', topics: '域的原理 · 常用域 · 域开关与格式 · 嵌套域 · 更新与调试' },
      { n: 19, slug: 'ch19', title: '宏与 VBA 自动化', topics: '录制宏 · VBA 基础语法 · 对象模型 · 实用宏案例 · 宏安全与分发' },
      { n: 20, slug: 'ch20', title: '效率精通与疑难排查', topics: '快捷键体系 · 常见疑难问题速查 · 性能优化 · 跨平台与协作最佳实践' },
    ],
  },
]

export default function WordTutorialIndex() {
  return (
    <div style={s.page}>
      ...
      <div style={s.hero}>
        <div style={s.heroInner}>
          <div style={s.heroTags}>
            <span style={s.tag}>📘 出书级完整教程</span>
            <span style={s.tag}>📗 5 篇 20 章</span>
            <span style={s.tag}>📕 100+ 知识点</span>
            <span style={s.tag}>✅ 全平台适用</span>
          </div>
          <h1 style={s.heroTitle}>Word 从入门到精通</h1>
          <p style={s.heroDesc}>一部达到实体书标准的 Microsoft Word 完整教程。全书分 5 篇 20 章，覆盖从界面认识、字符段落排版、样式主题、长文档工程，到表格图形、邮件合并、修订协作、域与 VBA 自动化的全部核心知识。每一节都包含「概念原理 → 分步操作 → 实战案例 → 常见错误与避坑 → 进阶技巧」五大模块，既适合零基础系统学习，也可作为案头查阅手册。</p>
          <div style={s.statsRow}>
            {[{v:'20',l:'章节',i:'📖'},{v:'100+',l:'知识点',i:'💡'},{v:'5',l:'篇章',i:'📚'},{v:'全体',l:'适合人群',i:'👥'}].map(x=>(
      ...
      <div style={s.content}>
        <div style={s.intro}>
          <div style={s.introTitle}>📖 关于本书</div>
          <p style={{margin:0}}>本教程以「能上手、能查阅、能精通」为目标编写。建议初学者按篇章顺序循序渐进，每学完一节立即打开 Word 对照实操；有一定基础的读者可直接跳读所需章节。每章末尾提供「上一章 / 返回目录 / 下一章」导航，方便连续阅读。点击下方任意章节即可开始学习。</p>
        </div>

        {parts.map(part => (
          <div key={part.no} style={s.partCard}>
            <div style={s.partHeader}>
              <div style={s.partBadge}>{part.no}</div>
              <div>
                <div style={s.partTitle}>第{part.no}篇 · {part.title}</div>
                <div style={s.partDesc}>{part.desc}</div>
              </div>
            </div>
            <div style={s.chapterList}>
              {part.chapters.map(ch => (
                <Link key={ch.n} href={"/tutorials/word/" + ch.slug} style={s.chapterRow}>
                  <div style={s.chNum}>{ch.n}</div>
                  <div style={s.chBody}>
                    <div style={s.chTitle}>第{ch.n}章　{ch.title}</div>
                    <div style={s.chTopics}>{ch.topics}</div>
                  </div>
                  <div style={s.chArrow}>›</div>
                </Link>
              ))}
            </div>
          </div>
      ...
      <footer style={s.footer}>
        <p>© {new Date().getFullYear()} ZZGCopilot · Word 从入门到精通（完整版）· 5 篇 20 章</p>
      </footer>
    </div>
  )
}
