...
export async function GET() {
  try {
    ...
    const ownerWhere = canManageAllContent(access.role) ? '' : ' WHERE author_id = ?'
    const ownerAnd = canManageAllContent(access.role) ? '' : ' AND author_id = ?'
    const ownerArgs = canManageAllContent(access.role) ? [] : [access.userId]
    const [posts, users, comments, views, media, backup, scheduler, trend] = await Promise.all([
      db.prepare(`SELECT COUNT(*) AS total,
        ...
        SUM(CASE WHEN status = 'archived' THEN 1 ELSE 0 END) AS archived FROM posts${ownerWhere}`).bind(...ownerArgs).first<Record<string, number>>(),
      canManageAllContent(access.role) ? db.prepare('SELECT COUNT(*) AS count FROM users WHERE is_active = 1').first<{ count: number }>() : Promise.resolve({ count: 1 }),
      canManageAllContent(access.role) ? db.prepare('SELECT COUNT(*) AS count FROM comments').first<{ count: number }>() : Promise.resolve({ count: 0 }),
      db.prepare(`SELECT COALESCE(SUM(view_count), 0) AS count FROM posts WHERE status = 'published'${ownerAnd}`).bind(...ownerArgs).first<{ count: number }>(),
      db.prepare(`SELECT COUNT(*) AS count FROM media WHERE deleted_at IS NULL${canManageAllContent(access.role) ? '' : ' AND uploaded_by = ?'}`).bind(...ownerArgs).first<{ count: number }>(),
